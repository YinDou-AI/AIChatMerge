...
### 4.7 提示词重构：从裁判评论到博取众长（已实现）

**问题：** 原提示词让 AI 做「裁判评论」（分析观点、标注争议、给排名），用户看完既不知道具体内容，也不知道该去看哪个 AI。

**方案 A（已采用）：博取众长**
- 融合答案直接输出综合了各 AI 优质内容的完整回答
- 用户看完这一个答案就够了，不需要再看各个 AI
- 提示词从「你是裁判」改为「你是综合者」

**核心变化：**
```
旧：你是裁判，评估各模型回答，给出排名
新：你是综合者，从各模型回答中提取最优质的内容，整合成一个完整的最佳答案
```

**输出要求：**
- 先复述原始问题（便于后续回顾）
- 直接输出综合答案，不写「模型A说了X，模型B说了Y」
- 最终答案比任何单个模型都更完整、更有用

### 4.7.1 融合目标下拉框修复（已实现）

**问题：** 原生 `<select>` 点击后立即关闭，用户体验差。

**修复：** 改为自定义下拉菜单，使用 pointerdown 事件实现点击外部关闭（与添加面板菜单一致）。
...
### 22.5 GitHub Actions 全流程测试可行性分析

**问题：** CI 能否模拟用户发送操作，检测从发送到提取的完整流程？
...
| 限制 | 说明 | 能否解决 |
|------|------|---------|
| 登录态 | 所有 AI 平台都需要登录 | 可以，但有风险 |
| 验证码 | 部分平台登录时弹验证码 | 难以自动化 |
| 费用 | 每次测试消耗各平台 API 额度 | 需要付费账号 |
| 反爬 | CI 环境 IP 可能被标记 | 需要代理 |
| 超时 | AI 回答需要 30-120 秒 | CI 默认 6 小时够用 |

#### 22.5.3 登录态解决方案

| 方案 | 说明 | 安全性 | 稳定性 |
|------|------|--------|--------|
| A. Cookie 注入 | 把登录 Cookie 存 GitHub Secrets | 中（Cookie 会过期） | 低 |
| B. 账号密码 | 存 Secrets，Playwright 自动登录 | 低（有验证码风险） | 低 |
| C. 只测不需要登录的部分 | 检查选择器、注入逻辑 | 高 | 高 |
| D. 本地测试 + CI 只做结构检测 | CI 检查选择器，本地人工验证全流程 | 高 | 高 |

#### 22.5.4 结论：推荐 D 方案

**CI 做（自动化）：**
- 选择器存在性检测（Playwright 打开公共页面）
- manifest.json 校验
- 代码 lint + 单元测试
- 每日定时 + PR 触发
...
**本地做（人工验证）：**
- 全流程测试（登录 + 发送 + 提取 + 融合）
- 本地 Playwright 脚本，手动运行

**原因：** 登录态在 CI 中太脆弱，Cookie 过期、验证码、IP 封禁都会导致测试频繁失败，维护成本远高于收益。

#### 22.5.5 本地自动化测试脚本（后续实现）
...
```js
// tests/e2e/full-flow.spec.mjs
// 本地运行，不依赖 CI
// Playwright 打开扩展 → 发送测试问题 → 验证提取 → 输出报告
```
