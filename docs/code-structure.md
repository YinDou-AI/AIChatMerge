...
| 调试日志接口 | `modules/debug-log.js` | no-op 兼容接口（正式版已剔除记录/下载） | `debug:*` |
...
selectPrompt() → applyPromptToInput()  // prompt-editor.js（点击时 {variable} 替换为 unified-input 当前内容后填入）
prependDefaultPrompt() → {variable} 发送时替换为输入内容（含 {variable} 为模板模式，否则约束模式前置）  // default-prompt.js
...
### 4.4.0.1 调试日志接口（debug-log.js，正式版为 no-op 兼容接口）
```
recordDebugLog(event, details) → void          // no-op（正式版已剔除日志录制）
downloadDebugLogs(options?, showToast?) → void  // no-op（下载/导出功能已剔除）
clearDebugLogs(showMessage?) → void             // no-op
setMergePanelIds(ids) → void                    // no-op
setDiscussionWillRun(value) → void              // no-op
getDebugSessionId() → string                    // 恒为 'diagnostics-disabled'
```

官方版（正式版）源码已直接采用打包隔离产物：`debug-log.js` 为
`debug-log.release.js` 的无副作用兼容接口（无 storage、无 downloads、无定时器、
无诊断状态）。业务模块仍依赖其稳定接口调用，但不会录制或下载任何调试日志。
日志接口不参与成功或失败判定；no-op 实现下发送、完成监控和提取行为完全一致。
...
```javascript
{
  ...
  variables: string[],  // 变量列表（自动检测；{variable} 在点击提示词时替换为 unified-input 当前内容，默认提示词在发送时替换为输入内容）
