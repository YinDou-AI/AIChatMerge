...
describe('prompting backward-compat layer', () => {
  ...
  it('both paths export the same documented public API count', () => {
    // Both should export exactly 20 business action functions
    expect(Object.keys(promptingIndex).length).toBe(20);
    expect(Object.keys(promptLibraryCompat).length).toBe(20);
  });
});
