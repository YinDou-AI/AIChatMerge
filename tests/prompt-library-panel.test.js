...
describe('prompt library panel wiring', () => {
  ...
  it('replaces {variable} with the unified input content on prompt selection', () => {
    expect(promptEditorSource).toContain("content.replace(/\\{variable\\}/g, inputContent)");
    expect(promptEditorSource).toContain("document.getElementById('unified-input')");
    expect(promptEditorSource).not.toContain('showVariableModal');
    expect(promptEditorSource).not.toContain('applyVariables');
    expect(promptEditorSource).not.toContain('closeVariableModal');
  });
