...
export function buildFinalMergePrompt(question, previousMergedAnswer, reviewAnswers) {
  ...
  if (isEn) {
    return `You are the final answer generator. Produce one complete, user-ready final answer: keep the previous merged result as the main body and apply only clear, valid corrections or additions from the review comments. Current date: ${today}
[Original Question]
${question}
...
${reviewParts}
Rules:
1. Output one complete final answer (title, body, scores) with the previous merged result as the main body, integrating valid corrections and additions; never output a summary, review, or discussion of the review comments
2. Treat "agree / no new corrections" responses as confirmation only; do not include them in the body
3. Prioritize recent and reliable information; make a judgment when review comments conflict
4. Use Markdown, no tables
5. The first line must be the title, format: Title: title within 10 words; then output the complete body; the last line must be the scores, starting with "Model scores:", full score is 10, format: Model scores: ModelName=score, ModelName=score`.replace(/\n{2,}/g, '\n');
  }

  return `你是最终答案生成器，负责输出一份可直接交付用户的完整最终答案，将上一版融合结果作为正文主体，仅采纳各模型复核意见中明确有效的修正与补充。当前日期：${today}
[原始问题]
${question}
[上一版融合结果]
${normalizeAnswerForMerge(previousMergedAnswer)}
...
${reviewParts}
规则：
1. 输出一份完整最终答案（标题、正文、评分），以[上一版融合结果]为正文主体，融合有效修正与补充后整体输出；禁止输出复核意见的罗列、评论或讨论过程
2. 对“同意当前融合结果、无新增修正”类回答，只视为确认，不写入正文
